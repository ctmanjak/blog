<?
	...
	mysql_connect("121.187.196.120", "user", "");
	mysql_select_db("blog");
	
	//owner
	$owner = basename(dirname(__FILE__));
	$sql = mysql_query("select user.id, nickname, blog_title from user left join user_info on user.id=user_info.id where username='$owner'");
	...
	else
	{
		...
		$bookmark_name = "$owner_nick l $blog_title";
