<?
	...
	if($addcomment == 1)
	{
		...
		mysql_query("insert into comment values($post_id, $owner_id, $log_id, $comment_id, '$com_content')");
...
<html>
	...
	{?>
		...
		<tr><td colspan=2 class="post_content"><?=nl2br($data[post_content])?></td></tr>
		<tr><td colspan=4 class="board_line"></td></tr>
		</table>
		<?if($owner_id == $log_id) 
		{?>
		...
		<div class="comment">
			<?
			$sql = mysql_query("select com_user_id, username, nickname, comment_id, com_content from user, user_info, comment where user.id=user_info.id and user_info.id=comment.com_user_id and post_id=$post_id and user_id=$owner_id order by comment_id");
			while($data = mysql_fetch_array($sql))
			{?>
				<div class="comment_main">
					<div class="comment_header">
						<a href="../<?=$data[username]?>" target="_parent"><?=$data[nickname]?></a>
...
<?}
	...
	{?>
		...
		while($data = mysql_fetch_array($sql))
		{?>
			<tr><td align="center"><?=$data[post_id]?></td><td><a href="board.php?readpost=1&post_id=<?=$data[post_id]?>"><?=$data[post_name]?><?if(!empty($data[comment_num]))print "<span style='font-size:0.8em;color:#ff4500;font-weight:bold'>[$data[comment_num]]</span>";?></a></td><td align="center"><?=$data[post_date]?></td><td align="center"><?=$data[view_num]?></td></tr>
			<tr><td colspan=4 class="board_line"></td></tr>
		<?}?>
		</table>
